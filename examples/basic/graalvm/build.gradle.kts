        ...
        if (!profileFile.isFile) {
            throw GradleException(
                "Missing PGO profile: ${profileFile.absolutePath}. " +
                    "Run :examples:basic:graalvm:basic_train_desktop_graalvm_pgo first."
            )
        }
        ...
        if (staleProfiles.isNotEmpty()) {
            throw GradleException(
                "Found stale PGO profile(s): ${staleProfiles.joinToString { it.name }}. " +
                    "Run :examples:basic:graalvm:basic_train_desktop_graalvm_pgo again to clean and regenerate the profile."
            )
        }
    }
}
