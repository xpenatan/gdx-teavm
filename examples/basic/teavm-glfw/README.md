...
**Debug build**
```
gradlew :examples:basic:teavm-glfw:basic_build_teavm_glfw_debug
```

**Release build**
```
gradlew :examples:basic:teavm-glfw:basic_build_teavm_glfw_release
```
...
```
gradlew :examples:basic:teavm-glfw:basic_generate_teavm_glfw
```

The existing task name still works as a debug-build alias:
```
...
```
gradlew :examples:basic:teavm-glfw:basic_run_teavm_glfw_debug
```
...
```
gradlew :examples:basic:teavm-glfw:basic_run_teavm_glfw_debug
gradlew :examples:basic:teavm-glfw:basic_run_teavm_glfw_release
gradlew :examples:basic:teavm-glfw:basic_run_teavm_glfw_release_console
```

The run tasks generate C code, compile the native executable, and start it. On Windows, the release console task opens a console window and runs the native executable inside it, so stdout/stderr logs are visible while the app is running.
