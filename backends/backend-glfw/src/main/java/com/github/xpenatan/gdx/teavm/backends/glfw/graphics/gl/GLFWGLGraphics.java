...
public class GLFWGLGraphics extends AbstractGraphics implements Disposable {
    ...
    GLFW.GLFWFramebufferSizeCallback resizeCallback = Function.get(GLFW.GLFWFramebufferSizeCallback.class, GLFWGLGraphics.class, "resizeCallback");
...
//        }
    ...
    public GLFWGLGraphics(GLFWWindow window) {
        this.window = window;
        if (window.getConfig().glEmulation == GLFWApplicationConfiguration.GLEmulation.GL32) {
            this.gl20 = this.gl30 = this.gl31 = this.gl32 = new GLFWGL32();
        } else if (window.getConfig().glEmulation == GLFWApplicationConfiguration.GLEmulation.GL31) {
            this.gl20 = this.gl30 = this.gl31 = new GLFWGL32();
        } else if (window.getConfig().glEmulation == GLFWApplicationConfiguration.GLEmulation.GL30) {
    ...
    public void updateFramebufferInfo() {
        ...
        GLFWGLGraphics.this.logicalWidth = tmpBuffer[0];
        GLFWGLGraphics.this.logicalHeight = tmpBuffer2[0];
    ...
    public float getPpcX() {
        ...
        GLFW.getMonitorPhysicalSize(monitor.monitorHandle, tmpBuffer, tmpBuffer2);
    ...
    public Monitor getPrimaryMonitor() {
        return GLFWApplicationConfiguration.toNativeMonitor(GLFW.getPrimaryMonitor());
    }
    ...
    public Monitor[] getMonitors() {
        ...
        for (int i = 0; i < glfwMonitors.length; i++) {
            monitors[i] = GLFWApplicationConfiguration.toNativeMonitor(glfwMonitors[i]);
        }
        return monitors;
    }
    ...
    public boolean setWindowedMode(int width, int height) {
        ...
        if (!isFullscreen()) {
            ...
            if (width != logicalWidth || height != logicalHeight) {
                ...
                newPos = GLFWApplicationConfiguration.calculateCenteredWindowPosition((NativeMonitor) getMonitor(), width, height);
            }
            ...
            if (width != windowWidthBeforeFullscreen || height != windowHeightBeforeFullscreen) { // center the window since its size
                // changed
                GridPoint2 newPos = GLFWApplicationConfiguration.calculateCenteredWindowPosition((NativeMonitor) getMonitor(), width,
                        height);
    ...
    public void setUndecorated(boolean undecorated) {
        ...
        GLFW.setWindowAttrib(window.getWindowHandle(), GLFW.GLFW_DECORATED, undecorated ? GLFW.GLFW_FALSE : GLFW.GLFW_TRUE);
    }

    @Override
    public void setResizable(boolean resizable) {
        getWindow().getConfig().setResizable(resizable);
        GLFW.setWindowAttrib(window.getWindowHandle(), GLFW.GLFW_RESIZABLE, resizable ? GLFW.GLFW_TRUE : GLFW.GLFW_FALSE);
    }
    ...
    public Cursor newCursor(Pixmap pixmap, int xHotspot, int yHotspot) {
        return new GLFWCursor(getWindow(), pixmap, xHotspot, yHotspot);
    }

    @Override
    public void setCursor(Cursor cursor) {
        GLFW.setCursor(getWindow().getWindowHandle(), ((GLFWCursor) cursor).glfwCursor);
    }

    @Override
    public void setSystemCursor(SystemCursor systemCursor) {
        GLFWCursor.setSystemCursor(getWindow().getWindowHandle(), systemCursor);
    }
    ...
    public static class NativeDisplayMode extends DisplayMode {
        ...
        public NativeDisplayMode(long monitor, int width, int height, int refreshRate, int bitsPerPixel) {
    ...
    public static class NativeMonitor extends Monitor {
        public final long monitorHandle;

        public NativeMonitor(long monitor, int virtualX, int virtualY, String name) {
