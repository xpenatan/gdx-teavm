...
import emu.com.badlogic.gdx.assets.loaders.TTextureLoader;
import java.util.HashMap;
import java.util.Map;

public class TTexture extends GLTexture {
    private static AssetManager assetManager;
    final static Map<Application, Array<TTexture>> managedTextures = new HashMap<Application, Array<TTexture>>();
    ...
    public TTexture(String internalPath) {
        this(Gdx.files.internal(internalPath));
    }

    public TTexture(FileHandle file) {
        this(file, null, false);
    }

    public TTexture(FileHandle file, boolean useMipMaps) {
        this(file, null, useMipMaps);
    }

    public TTexture(FileHandle file, Pixmap.Format format, boolean useMipMaps) {
        this(TextureData.Factory.loadFromFile(file, format, useMipMaps));
    }

    public TTexture(Pixmap pixmap) {
        this(new PixmapTextureData(pixmap, null, false, false));
    }

    public TTexture(Pixmap pixmap, boolean useMipMaps) {
        this(new PixmapTextureData(pixmap, null, useMipMaps, false));
    }

    public TTexture(Pixmap pixmap, Pixmap.Format format, boolean useMipMaps) {
        this(new PixmapTextureData(pixmap, format, useMipMaps, false));
    }

    public TTexture(int width, int height, Pixmap.Format format) {
        this(new PixmapTextureData(new Pixmap(width, height, format), null, false, true));
    }

    public TTexture(TextureData data) {
        this(GL20.GL_TEXTURE_2D, Gdx.gl.glGenTexture(), data);
    }

    protected TTexture(int glTarget, int glHandle, TextureData data) {
    ...
    private static void addManagedTexture (Application app, TTexture texture) {
        Array<TTexture> managedTextureArray = managedTextures.get(app);
        if (managedTextureArray == null) managedTextureArray = new Array<TTexture>();
    ...
    public static void invalidateAllTextures (Application app) {
        ...
        if (assetManager == null) {
            for (int i = 0; i < managedTextureArray.size; i++) {
                TTexture texture = managedTextureArray.get(i);
                texture.reload();
            }
        } else {
            ...
            // asset manager.
            Array<TTexture> textures = new Array<TTexture>(managedTextureArray);
            for (TTexture texture : textures) {
                ...
                if (fileName == null) {
                    ...
                    TTextureLoader.TextureParameter params = new TTextureLoader.TextureParameter();
                    ...
                    assetManager.load(fileName, TTexture.class, params);
                }
            }
