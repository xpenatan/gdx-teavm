...
public class DragomeSound implements Sound {
    ...
    public DragomeSound(FileHandle file) {
        soundFile = file;
        sounds = new DragomeMusic[MAX_SOUNDS];
        sounds[0] = new DragomeMusic(file);
        soundIndex = 0;
    }
    ...
    private long play(float volume, float pitch, float pan, boolean loop) {
        ...
        if(soundId >= 0) {
            ...
            if(sounds[soundId] == null) {
                sounds[soundId] = new DragomeMusic(soundFile);
            }
