 ...
 * </ul>
 * </p> */
final class DirectReadOnlyFloatBufferAdapter extends XFloatBuffer {
// implements DirectBuffer {

    static XFloatBuffer wrap (XDirectByteBuffer byteBuffer) {
        return new DirectReadOnlyFloatBufferAdapter((XDirectByteBuffer)byteBuffer.slice());
    }

    private final XDirectByteBuffer byteBuffer;
    private final Float32ArrayWrapper floatArray;

    DirectReadOnlyFloatBufferAdapter (XDirectByteBuffer byteBuffer) {
    ...
    public XFloatBuffer duplicate () {
        DirectReadOnlyFloatBufferAdapter buf = new DirectReadOnlyFloatBufferAdapter((XDirectByteBuffer)byteBuffer.duplicate());
        buf.limit = limit;
        buf.position = position;
        buf.mark = mark;
        return buf;
...
// }
    ...
    public XFloatBuffer slice () {
        ...
        XFloatBuffer result = new DirectReadOnlyFloatBufferAdapter((XDirectByteBuffer)byteBuffer.slice());
        byteBuffer.clear();
        return result;
    }
