package emucom.badlogic.gdx.utils;
...
public class Timer {
    ...
    public Task scheduleTask(Task task, float delaySeconds, float intervalSeconds, int repeatCount) {
        ...
        task.executeTimeMillis = emucom.badlogic.gdx.utils.TimeUtils.nanoTime() / 1000000 + (long)(delaySeconds * 1000);
    ...
    static class TimerThread implements Runnable, LifecycleListener {
        ...
        public TimerThread(TimerWrapper timerWrapper) {
            this.timerWrapper = timerWrapper;
            timerWrapper.setRunnable(this);
            Gdx.app.addLifecycleListener(this);
            resume();
        }

        public void schedule(int delayMillis) {
            timerWrapper.schedule(delayMillis);
        }
        ...
        public void run() {
            synchronized(instances) {
                ...
                long timeMillis = emucom.badlogic.gdx.utils.TimeUtils.nanoTime() / 1000000;
        ...
        @Override
        public void resume() {
            long delayMillis = emucom.badlogic.gdx.utils.TimeUtils.nanoTime() / 1000000 - pauseMillis;
            synchronized(instances) {
                for(int i = 0, n = instances.size; i < n; i++)
