...
class TCharBufferOverByteBufferBigEndian extends TCharBufferOverByteBuffer {
    ...
    char getChar(int index) {
        int value = ((byteByffer.array.get(start + index * 2) & 0xFF) << 8)
                | (byteByffer.array.get(start + index * 2 + 1) & 0xFF);
        return (char) value;
    }

    @Override
    void putChar(int index, char value) {
        byteByffer.array.set(start + index * 2, (byte) (value >> 8));
        byteByffer.array.set(start + index * 2 + 1, (byte) value);
    }
